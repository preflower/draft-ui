export { default as ScrollTabsContainer } from './ScrollTabsContainer.vue'
export { default as ScrollTabsContent } from './ScrollTabsContent.vue'
export { default as ScrollTabsIndicator } from './ScrollTabsIndicator.vue'
export { default as ScrollTabsList } from './ScrollTabsList.vue'
export { default as ScrollTabsRoot } from './ScrollTabsRoot.vue'
export { default as ScrollTabsTrigger } from './ScrollTabsTrigger.vue'
export * from './types'
export * from './useScrollTabs'
